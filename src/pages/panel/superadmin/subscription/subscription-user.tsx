...
const SubscriptionUser = () => {
  ...
  const { data, error, mutate, isValidating } = useSWRAPI(
    `payment/all-subscription/history?perPage=10&pageNo=${currentPage}`
  );
  const [openPayment, setOpenPayment] = useState(false);
  const [openOrder, setOpenOrder] = useState(false);

  return (
    <TenantLayout title="Subscription User">
          ...
          <MaterialTable
            ...
            data={data?.data?.data?.map((property: any, i: number) => {
              return {
                ...
                timestamp: property?.planCreated
                  ? dayjs(property?.planCreated).format("LLL")
                  : "Not available",
              };
            })}
            components={{
            ...
            columns={[
              ...
              },
              {
                ...
                render: (row) => (
                  <div className="flex gap-4">
                    ...
                    >
                      <button className="btn-one ">Billing Details</button>
                    </div>
                    <div
              ...
              },
              {
                ...
                field: "timestamp",
                render: ({ order }: any) => (
                  <>{dayjs(order?.planCreated).format("lll")}</>
                ),
              },
            ]}
          />
...
export default withProtectedSuperAdmin(SubscriptionUser);
