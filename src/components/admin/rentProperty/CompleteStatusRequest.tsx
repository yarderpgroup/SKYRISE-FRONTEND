...
const CompleteStatusRequest = ({ activeType }: any) => {
  ...
  const { data, error, isValidating, mutate } = useSWRAPI(
    `application/landlord/pending/${propertyID}?status=true`
  );
  const GoToViewDetails = (ID: string) => {
    router.push(`/panel/admin/rent/${propertyID}/application/${ID}`);
  };
            ...
            {data?.data?.data?.map((item: any) => (
              <div className="md:col-span-3 col-span-12 flex flex-col bg-white rounded-lg overflow-hidden shadow-[rgba(50,_50,_105,_0.15)_0px_2px_5px_0px,_rgba(0,_0,_0,_0.05)_0px_1px_1px_0px] common-transition">
                ...
                <div className="flex w-full gap-2 p-4">
                  <button
                    className="btn-one w-1/2"
                    onClick={() => GoToViewDetails(item?.tenantId)}
                  >
                    View Details
                  </button>
                  <button
                    className="btn-two w-1/2"
...
export default CompleteStatusRequest;
