...
const EditUtility = ({ open, onClose, mutate, isData }: Props) => {
  ...
  const AddBankSchema = [
    {
      ...
      options: [
        ...
        {
          id: 6,
          label: "TRASH COLLECTION FEE",
          value: "TRASH COLLECTION FEE",
        },
        {
          id: 7,
          label: "OTHERS",
  ...
  const handleSend = async (values: any) => {
    try {
      ...
      const response = await put({
        path: `utility/update-bill/${propertyID}`,

        isAlert: true,
        body: JSON.stringify({
          utilityId: isData?._id,
          type: values?.type,
