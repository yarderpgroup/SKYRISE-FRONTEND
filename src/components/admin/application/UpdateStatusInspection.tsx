  ...
  const issueSchema = [
    {
      ...
      initialValue: activeData?.status,
      type: "select",
  ...
  const handleEditRequest = async (values: any) => {
    try {
      ...
      const response = await put({
        path: `inspection/landlord/status/${propertyID}?inspectionId=${activeData?._id}&status=${isStatus}`,
        isAlert: true,
      });
      setIsLoading(false);
      onClose();
...
export default UpdateStatusInspection;
