...
import AddDocumentSchema from "schemas/AddDocumentSchema";
...
const EditDocumentDrawer = ({ open, onClose, mutate }: Props) => {
  const handleSend = async (values: any) => {};
  const initialValues = AddDocumentSchema.reduce(
  ...
  const validationSchema = AddDocumentSchema?.reduce(
    (accumulator, currentValue) => {
            ...
            {(formik) => (
              <Form>
                <div className="grid grid-cols-1 gap-4 w-full justify-center items-center justify-items-center  ">
                  {AddDocumentSchema?.map((inputItem) => (
