...
import EditLocationDetails from "schemas/EditLocationDetails";
...
const RegisteredPropertyDrawer = ({ open, onClose, mutate }: Props) => {
  const handleSend = async (values: any) => {};
  const initialValues = EditLocationDetails.reduce(
  ...
  const validationSchema = EditLocationDetails?.reduce(
    (accumulator, currentValue) => {
            ...
            {(formik) => (
              <Form>
                <div className="grid grid-cols-1 gap-4 w-full justify-center items-center justify-items-center  ">
                  {EditLocationDetails?.map((inputItem) => (
...
export default RegisteredPropertyDrawer;
