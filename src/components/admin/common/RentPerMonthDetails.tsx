...
import { CollectRentdetails } from ".";
import { PaginationButton } from "components/core";

const RentPerMonthDetails = ({ rentDetails, propertyID }: any) => {
  ...
  const { data, error, mutate, isValidating } = useSWRAPI(
    `lease/landlord/get-all-rent/${propertyID}?tenantId=${rentDetails}&perPage=10&pageNo=${currentPage}`
  );
...
export default RentPerMonthDetails;
