...
const DisclosuresDetailsAdd = ({
  ...
  const initialValues = AddDisclosures.reduce(
  ...
  const validationSchema = AddDisclosures?.reduce(
    (accumulator: any, currentValue: any) => {
            ...
            >
              {(formik) => (
                <Form className="w-full grid grid-cols-12 gap-2 md:gap-4">
                  {AddDisclosures.map((inputItem: any) => (
...
export default DisclosuresDetailsAdd;
