...
const AttachmentDetails = () => {
  ...
  const { data, error, mutate, isValidating } = useSWRAPI(
    `lease/get-all/document/${ID}${tenant}`
  );
  const myDocumentArr = data?.data?.data;
  ...
  const handelSend = () => {
    router.push(`/panel/admin/rent/${ID}/lease-details/leassor-info${tenant}`);
  };
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          response = await remove({
            path: `lease/delete/document/${ID}?documentId=${row?._id}`,
            isAlert: true,
          });
          mutate();
        }
              ...
              {myDocumentArr?.length > 0 ? (
                <div className="w-full grid grid-cols-12 gap-4 md:gap-8">
                  {myDocumentArr?.map((item: any) => (
                    <div className="2xl:col-span-3 md:col-span-3 col-span-6 h-48 md:h-60 p-2 md:p-5 shadow-[rgba(50,_50,_105,_0.15)_0px_2px_5px_0px,_rgba(0,_0,_0,_0.05)_0px_1px_1px_0px] bg-white rounded-md flex items-center justify-center flex-col gap-3 relative group">
                      ...
                      <div className="absolute md:right-10 right-2 top-1 md:top-4 !z-[88]">
                        <Delete
              ...
              >
                {(formik) => (
                  <Form>
                    ...
                    <div className="flex items-center col-span-12  justify-center flex-col gap-2 py-8"></div>
                  </Form>
                )}
              </Formik>
            </div>
...
export default withProtectedLandlord(
  withProtectedSubscription(AttachmentDetails)
);
