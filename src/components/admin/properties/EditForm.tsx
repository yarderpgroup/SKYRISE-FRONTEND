...
import EditOtherInformationSchema from "schemas/EditOtherInformationSchema";
...
const EditForm = ({ open, onClose, mutate }: Props) => {
  const handleSend = async (values: any) => {};
  const initialValues = EditOtherInformationSchema.reduce(
  ...
  const validationSchema = EditOtherInformationSchema?.reduce(
    (accumulator, currentValue) => {
            ...
            {(formik) => (
              <Form>
                <div className="grid grid-cols-1 gap-4 w-full justify-center items-center justify-items-center  ">
                  {EditOtherInformationSchema?.map((inputItem) => (
