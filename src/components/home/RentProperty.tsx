...
const RentProperty = ({ featureData, isValidating, mutate }: any) => {
    ...
    <section className="w-full bg-white custom-container md:pt-16 pt-8">
      <div className="flex flex-col gap-4 md:gap-6">
        <p className="flex gap-2 title-styling font-semibold text-themeDarkGray">
          <span className="relative after:absolute after:-bottom-1 after:left-0 after:w-full after:h-[3px] after:bg-theme ">
            Rent
          </span>
          <span className="">Property</span>
        </p>
        <p className="md:leading-7 leading-6 md:text-lg text-base relative text-themeDarkGray">
          ...
          consultation.
          <div className="md:absolute right-20 md:flex hidden gap-3">
            <div
              onClick={handleNextSlide}
              className={`bg-white flex items-center justify-center cursor-pointer border-theme w-10 border-2 rounded-full h-10`}
            >
              <IconButton>
            ...
            <div
              ...
              className={`gradientButton text-white flex items-center justify-center cursor-pointer w-10 border-2 rounded-full h-10`}
            >
              <IconButton>
        ...
        <div className="w-full relative pt-3 md:pt-10 pb-3 md:pb-0">
          {isValidating ? (
            ...
            <section className="testimonial-slick company-slick-slider store-slider overflow-hidden">
              <Slider ref={ref} {...settings} className="our-store-dots">
                {featureData?.data?.data?.map((rentItem: any) => (
                  <PropertyCard
                    key={rentItem.id}
                    curElm={rentItem}
                    mutate={mutate}
