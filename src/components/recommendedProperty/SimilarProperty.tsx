...
const SimilarProperty = ({ propertyId, type }: Props) => {
  const { user } = useAuth();
  ...
  return (
    <section className="w-full flex flex-col text-themeDarkGray py-4 md:py-8 gap-2 md:gap-4">
      <p className="md:text-base text-sm font-semibold md:text-left text-center">
        SIMILAR IN FEATURES RECOMMENDED PROPERTIES
      </p>
